...
} from './schemas/produitendommage.schema';
import { Model } from 'mongoose';
import { InjectModel } from '@nestjs/mongoose';
import {
  VenteProduitendommage,
  VenteProduitendommageDocument,
} from './schemas/venteproduitendommage.schema';
...
} from './schemas/produitendommagestock.schema';

@Injectable()
export class ProduitendommageService {
  constructor(
    @InjectModel(Produitendommage.name)
    private readonly productendoModel: Model<ProduitendommageDocument>,
    @InjectModel(VenteProduitendommage.name)
    private readonly venteproductendoModel: Model<VenteProduitendommageDocument>,
    @InjectModel(Produitendommagestock.name)
    private readonly productendostockModel: Model<ProduitendommagestockDocument>,
  ...
  async create(createProduitendommageDto: CreateProduitendommageDto) {
    const createproductendommage = await this.productendoModel.create(
      createProduitendommageDto,
    );
    if (createproductendommage) {
      const getstock = await this.productendostockModel
        .findOne({ productId: createProduitendommageDto.productId })
        .exec();
      if (getstock == null) {
        const createstock = {
        ...
        await this.productendostockModel
          .findByIdAndUpdate(
            { _id: getstock._id },
            { $set: updatestock },
            { new: true },
      ...
      if (createProduitendommageDto.origine == 'bureau') {
        ...
        const product = await this.stockagenceService.findagenceproduitendom(
          createProduitendommageDto.entrepot_or_agence,
          createProduitendommageDto.productId,
        );
        ...
        if (product.quantity - createProduitendommageDto.quantity >= 0) {
          ...
          throw new NotFoundException(
            'la quantité de ce produit est insuffisant dans ce bureau',
          );
        }
      } else if (createProduitendommageDto.origine == 'entrepôt national') {
        const product = await this.stockpaysService.findpaysproduit(
          createProduitendommageDto.productId,
          createProduitendommageDto.entrepot_or_agence,
        );
        if (product.quantity - createProduitendommageDto.quantity >= 0) {
          ...
          await this.stockpaysService.updateStockpaysproduit(
            product._id.toString('hex'),
            updateProduitDto,
          );
          ...
          throw new NotFoundException(
            'la quantité de ce produit est insuffisant dans cet entrepôt',
          );
        }
      } else {
        const product =
          await this.stockentrepotService.findproduitstockentrepot(
            createProduitendommageDto.productId,
          );
        if (product.quantity - createProduitendommageDto.quantity < 0) {
          throw new NotFoundException(
            "la quantité de ce produit est insuffisant dans l' entrepôt international",
          );
        }
        const stockentrepot = {
          productId: createProduitendommageDto.productId,
        ...
        this.stockentrepotService.updatestockentrepot(
          product._id.toString('hex'),
          stockentrepot,
        );
  ...
  async findAll() {
    const Products = await this.productendostockModel
      .find()
      .populate('productId')
      .exec();
    return Products;
  ...
  async findAllDetailByProduct(productId: string) {
    const Products = await this.productendoModel
  ...
  async findOne(id: string) {
    const product = await this.productendostockModel
  ...
  async createvente(
    createventeProduitendommageDto: CreateVenteProduitendommageDto,
  ) {
    const product = await this.productendostockModel
      .findOne({ productId: createventeProduitendommageDto.productId })
      .exec();
    console.log(createventeProduitendommageDto);
    console.log(product);
    if (product.quantity >= createventeProduitendommageDto.quantity) {
      const createproductendommage = await this.venteproductendoModel.create(
      ...
      if (createproductendommage) {
        ...
        await this.productendostockModel
          .findByIdAndUpdate(
            { _id: product._id },
            { $set: updateproductendo },
            { new: true },
          )
          .exec();

        const getPaysCaMois = await this.payscaservice.findOnePaysCamoisExist(
          createventeProduitendommageDto.countryId,
          createventeProduitendommageDto.mois,
          createventeProduitendommageDto.annee,
        );
        ...
        if (getPaysCaMois != null) {
          const upadateinfopaysCaMois = {
            ...
            caTotal:
              createventeProduitendommageDto.prix + getPaysCaMois.caTotal,
          };
        ...
        if (getPaysCaAnnee != null) {
          const upadateinfopaysCaYear = {
            ...
            caTotal:
              createventeProduitendommageDto.prix + getPaysCaAnnee.caTotal,
          };
      ...
      throw new NotFoundException(
        'stock insuffisant disponible est inférieur à quantité saisie',
      );
    }
  }

  findAllVente() {
    return this.venteproductendoModel
      .find()
      .populate('productId')
      .populate('mois')
      .populate('annee');
