...
export class AffectationService {
  ...
  async create(createAffectationDto: CreateAffectationDto) {
    for (let i = 0; i < createAffectationDto.managerId.length; i++) {
      const alreadyExists = await this.affectationModel
        .exists({ managerId: createAffectationDto.managerId[i]._id })
        .lean();
      if (alreadyExists) {
        throw new ConflictException(
          `Il y a déjà des managers cet bureau existe déjà dans la base de données`,
        );
      }
      ...
      const createdAffection = await this.affectationModel.create(
        affectationvalue,
      );
      if (createdAffection) {
        ...
        this.managerService.updateStatut(
          createAffectationDto.managerId[i]._id,
          updateStatusDto,
        );
      }
    }
  ...
  async findManager(bureauId: string) {
    const manager = await this.affectationModel
  ...
  async findManager_bureau(bureauId: string) {
    ...
    for (let i = 0; i < manager.length; i++) {
      if (
        manager[i].managerId['grade'] == 'Manager' ||
        manager[i].managerId['grade'] == 'Manager Superviseur de Zone' ||
        manager[i].managerId['grade'] == 'Manager Chef de section'
      ) {
  ...
  async remove(id: string) {
    ...
    const annuleAffectation = await this.managerService.updateStatut(
